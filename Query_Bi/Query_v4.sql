...
SELECT 
    ...
    DATEDIFF(DAY, p.fecha, f.fecha) AS diferencia_dias
FROM LOSGDS.Pedido p
JOIN LOSGDS.Detalle_Pedido dp     ON dp.det_ped_pedido = p.id_pedido
JOIN LOSGDS.Detalle_Factura df    ON df.det_fact_det_pedido = dp.id_det_pedido
JOIN LOSGDS.Factura f             ON f.id_factura = df.det_fact_factura
...
SELECT DISTINCT 
    Factura_Fecha, 
    Pedido_Fecha,
    DATEDIFF(DAY, Pedido_Fecha, Factura_Fecha) AS diferencia_dias
FROM gd_esquema.Maestra
WHERE 
    --Factura_Fecha < Pedido_Fecha and 
	Pedido_Numero = 56380973
--10372 fact y ped
/*
SELECT 
